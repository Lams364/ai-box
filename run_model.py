...
def set_model_name(model_name: str):
    global MODEL, TOKENIZER, DEVICE, MODEL_NAME  # Reference the globals
    response = create_model(model_name)
...
def change_token():
    ...
    return jsonify({'completed': completed, "token" : TOKEN})

@app.route('/model_info')
def model_info():
    return jsonify({'model': MODEL_NAME, "token" : TOKEN, "device": str(DEVICE)})
